...
public class FindSmallestWindow {
    ...
    static int minWindow(String s1, String s2, int m, int n) {
        ...
        while (j < m) {
            ...
            // Until the count does not becomes non-zero
			// Keep remove charcaters to reduce the window size
			// (These characters are basically the extra or non required characters)			
            while (count == 0) {
                if (minLength > j - i + 1) {
                    minLength = j - i + 1; // Get min window size
                    start = i; // Start index of min window
                    end = j; // End index of min window
                }
                ...
                if (map.containsKey(ch)) {
                    ...
                    // If previously count was zero and bveomes non zero later
                    // Increase count by 1
                    if (value == 0) {
                        count++;
                    }
